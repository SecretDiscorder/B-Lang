...
class Parser:
    ...
    def statement(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, "HENTI"):
            res.register_advancement()
            self.advance()
        ...
        if res.error:
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    "Expected 'KEMBALI', 'LANJUT', 'HENTI', 'TIPE', 'JIKA', 'TERULANG', 'KEULANG', 'ARTI', int, float, identifier, '+', '-', '(', '[' or 'BUKAN'",
                )
            )
    ...
    def expr(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, "TIPE"):
            res.register_advancement()
            self.advance()
        ...
        node = res.register(
            self.bin_op(self.comp_expr, ((TT_KEYWORD, "DAN"), (TT_KEYWORD, "ATAU")))
        )
        ...
        if res.error:
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    "Expected 'TIPE', 'JIKA', 'TERULANG', 'KEULANG', 'ARTI', int, float, identifier, '+', '-', '(', '[' or 'BUKAN'",
                )
            )
        ...
        if res.error:
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    "Expected int, float, identifier, '+', '-', '(', '[', 'JIKA', 'TERULANG', 'KEULANG', 'ARTI' or 'BUKAN'",
                )
            )
    ...
    def atom(self):
        ...
        elif tok.matches(TT_KEYWORD, "TERULANG"):
            for_expr = res.register(self.for_expr())
            if res.error:
                return res
            return res.success(for_expr)

        elif tok.matches(TT_KEYWORD, "KEULANG"):
            while_expr = res.register(self.while_expr())
            if res.error:
                return res
            return res.success(while_expr)

        elif tok.matches(TT_KEYWORD, "ARTI"):
        ...
        return res.failure(
            InvalidSyntaxError(
                tok.pos_start,
                tok.pos_end,
                "Expected int, float, identifier, '+', '-', '(', '[', JIKA', 'TERULANG', 'KEULANG', 'ARTI'",
            )
        )
    ...
    def for_expr(self):
        ...
        if not self.current_tok.matches(TT_KEYWORD, "TERULANG"):
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
    ...
    def while_expr(self):
        ...
        if not self.current_tok.matches(TT_KEYWORD, "KEULANG"):
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
    ...
    def func_def(self):
        ...
        if not self.current_tok.matches(TT_KEYWORD, "ARTI"):
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
...
class BuiltInFunction(BaseFunction):
    ...
    def execute_find(self, exec_ctx):
        ...
        try:
            if isinstance(index, Number):  # Memastikan index adalah angka tunggal
                toint = int(index.value)  # Konversi Decimal menjadi integer
                if 0 <= toint < len(array.elements):
                    val_ = array.elements[toint]
                    if isinstance(val_, String):
                        return RTResult().success(
                            String(array.elements[toint])
                        )
                    return RTResult().success(
                        List(array.elements[toint]) if isinstance(array.elements[toint], list) else val_
                    )
                else:
                    raise IndexError(f"Index '{toint}' is out of range")
            else:
                raise RTError(
                    self.pos_start, self.pos_end, "Index must be a number", exec_ctx
                )
        except (IndexError, RTError) as e:
            return RTResult().failure(
                RTError(
            ...
            return RTResult().failure(
                RTError(
                    self.pos_start, self.pos_end, f"An error occurred: {str(e)}", exec_ctx
                )
            )

    execute_find.arg_names = ["array", "index"]
...
global_symbol_table.set("MASUK", BuiltInFunction.input)
global_symbol_table.set("MASUK_ANGKA", BuiltInFunction.input_int)
global_symbol_table.set("CLEAR", BuiltInFunction.clear)
global_symbol_table.set("CLS", BuiltInFunction.clear)
global_symbol_table.set("ITU_NOMOR", BuiltInFunction.is_number)
global_symbol_table.set("ITU_HURUF", BuiltInFunction.is_string)
global_symbol_table.set("IS_LIST", BuiltInFunction.is_list)
global_symbol_table.set("ITU_ARTI", BuiltInFunction.is_function)
